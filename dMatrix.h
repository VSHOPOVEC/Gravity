...
typedef std::vector<std::vector<double_func>> func_2d_vec;
...
private:
    class MatrixRow {
    public:
        MatrixRow(T** in_matrix_arr, size_t row) : row(row), matrix_arr(in_matrix_arr) {}
...
T* Matrix<T>::operator[](size_t index) {
    if (index < 0 || columns < index) {
        ...
        return MatrixRow(matrix_arr, rows)[index];
    }
}
...
Matrix<T> Matrix<T>::InverseMatrix() const{
    ...
    if(deter == 0){
        ...
        */
        ...
        Matrix temp_matrix = (*this); Matrix Identity(true, this->rows,this->columns);
        size_t rows = temp_matrix.getRows();
        size_t columns = temp_matrix.getColumns();
        for(size_t i = 0; i < rows; i++){
            T coeff_val = 1 / temp_matrix[i][i];

            for(size_t k = 0; k < columns; k++){
                temp_matrix[i][k] *= coeff_val;
                Identity[i][k] *= coeff_val;
            }

            for(size_t i_1 = 0; i_1 < rows; i_1++){
                if(i == i_1) continue;
                T factor = temp_matrix[i_1][i];
                for(size_t j = 0; j < columns; j++){
                    temp_matrix[i_1][j] -= factor*temp_matrix[i][j];
                    Identity[i_1][j] -= factor*Identity[i][j];
                }
            }
        }
        return Identity;
...
class FuncMatrix:protected Matrix<std::function<double(double)>>{
public:

    FuncMatrix(const func_2d_vec& func_vec);

    Matrix<double> operator() (double t);

private:
    std::vector<std::vector<std::function<double(double)>>> func_vec;
    size_t rows, columns;
};

FuncMatrix::FuncMatrix(const func_2d_vec& func_vec):func_vec(func_vec){
    size_t rows = func_vec[0].size();
    size_t columns = InvalidValueArray(func_vec);
    this->rows = rows; this->columns = columns;
};

Matrix<double> FuncMatrix::operator() (double t){
    Matrix<double> temp_matr(this->rows, this->columns);
    for(size_t ind = 0; ind < rows; ++ind){
        for(size_t ind_1 = 0; ind_1 < columns; ++ind_1){
            temp_matr[ind][ind_1] = (func_vec[ind][ind_1])(t);
        }
    }
    return temp_matr;
}
